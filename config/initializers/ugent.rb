...
require 'template_options_controller'

class TemplateOptionsController

  # built in org selection method is weird, very weird
  #   e.g. you have to add "name" otherwise a match on "id" does not work
  def index
    ...
    # override - start
    p_params = params.permit(plan: [:research_org_id, :funder_id]).to_h.with_indifferent_access
    research_org_id = p_params.dig(:plan, :research_org_id)
    funder_id = p_params.dig(:plan, :funder_id)
    org = nil
    funder = nil

    if research_org_id.present?
      org = Org.where(id: research_org_id).first
    end
    if funder_id.present?
      funder = Org.where(id: funder_id).first
    end
    # override - end
